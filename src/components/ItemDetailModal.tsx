          ...
          <div id="modal-right-column" className="w-full md:w-96 flex-shrink-0 border-t md:border-t-0 md:border-l border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 relative flex flex-col">
                  ...
                  {onRefreshMetadata && (
                    <div className="space-y-1">
                      <button
                        ...
                        disabled={isRefreshing || (xApiStatus?.isRateLimited ?? false)}
                        className={`w-full px-3 py-1.5 text-xs rounded-md transition-colors flex items-center justify-center gap-1 disabled:cursor-not-allowed ${
                          xApiStatus?.isRateLimited 
                      ...
                      {xApiStatus?.isRateLimited && (
                        <p className="text-xs text-gray-500 dark:text-gray-400 text-center">
                          Will be available in {xApiStatus.minutesUntilReset} minute{xApiStatus.minutesUntilReset !== 1 ? 's' : ''}
                        </p>
                      )}
                    </div>
                  )}
            ...
            <div id="modal-right-column-bottom-bar" className="absolute bottom-0 inset-x-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-4">
              <div className="space-selector-button-container flex items-center gap-3">
                {/* Refresh Metadata Button - Available for all content types with URLs */}
                {onRefreshMetadata && currentItem.url && currentItem.content_type !== 'note' && currentItem.content_type !== 'x' && (
                  <button
                    onClick={handleRefreshMetadata}
                    disabled={isRefreshing}
                    className="p-2 text-green-600 dark:text-green-400 hover:text-green-700 dark:hover:text-green-300 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-md transition-colors flex-shrink-0 disabled:opacity-50 disabled:cursor-not-allowed"
